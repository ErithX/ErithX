...
export async function sendDailyContestDigest(
  ...
  if (!ENABLE_EMAILS) {
    console.log(`[KILL SWITCH] Emails are disabled. Skipped Digest Email to ${userEmail}`);
    return { success: true, skipped: true };
  }
  ...
  const subject = subjectLine || `Upcoming Contests this week for ${userName}`;
  ...
  try {
    const data = await resend.emails.send({
      ...
      to: userEmail,
      subject,
      react: <DigestEmail userName={userName} contests={contests} topResource={topResource} />,
    });
